# app.py
# Generative Abstract Poster with Color Selection
import random
import math
import numpy as np
...
FULL_PALETTE = {
    ...
    "ocean": (0.1,0.4,0.75),
    "sand": (0.9,0.75,0.5),
    "cloud": (0.95,0.95,0.95),
    "fire": (0.9,0.3,0.1),
    "ruby": (0.8,0.05,0.2),
    "gold": (1.0,0.85,0.0),
    "twilight": (0.3,0.15,0.5),
    "grass": (0.4,0.85,0.15),
    "brick": (0.65,0.2,0.15),
    "metal": (0.6,0.65,0.7),
    "grape": (0.5,0.2,0.7),
...
selected_colors = st.sidebar.multiselect(
    "Select Colors for Poster",
    options=list(FULL_PALETTE.keys()),
    default=list(FULL_PALETTE.keys())  # default: all colors
)

# Use only the selected colors
PALETTE = [FULL_PALETTE[c] for c in selected_colors] if selected_colors else list(FULL_PALETTE.values())
